...
func move(delta):
	...
	if (move_controller.right_pressed()):
		if (current_speed <= 0):
			current_speed = 0
		current_speed += SPEED / FRAMES_TO_FULL_SPEED * delta
		current_speed = clamp(current_speed, 0, SPEED * delta)
	elif (move_controller.left_pressed()):
		if (current_speed >= 0):
			current_speed = 0
		current_speed -= SPEED / FRAMES_TO_FULL_SPEED * delta
		current_speed = clamp(current_speed, -SPEED * delta, 0)
	else:
		if (abs(current_speed) <= SPEED / FRAMES_TO_STOP ):
			current_speed = 0
		else:
			current_speed += SPEED / FRAMES_TO_STOP * (-current_speed / abs(current_speed)) * delta
